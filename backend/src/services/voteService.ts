...
export class VoteService {
  ...
  // ============================================================
  // deleteVote — 删除投票（仅创建者，级联删除选项和投票记录）
  // ============================================================
  async deleteVote(
    voteId: string,
    userId: string,
    ...
    try {
      ...
      if (row.creator_id !== userId) {
        await trx.rollback();
        throw new AppError(40302, '仅投票发起者可删除投票');
      }
      if (row.team_id !== teamId) {
        await trx.rollback();
        throw new AppError(40302, '无操作权限');
      ...
      // 级联删除：options + user_votes 由 FK ON DELETE CASCADE 处理
      await trx('votes').where({ id: voteId }).del();
      await trx.commit();
    } catch (err) {
      await trx.rollback();
      throw err;
    ...
    // 清除 Redis 缓存
    await this.redis.del(`vote:${voteId}:tally`).catch(() => {});
    await this.redis.del(`vote:${voteId}:deadline`).catch(() => {});

    // WS 广播通知前端刷新
    this.io.to(`vote:${voteId}`).emit(`vote:${voteId}:deleted`, { vote_id: voteId });
